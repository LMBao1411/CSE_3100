...
t_pair *shortest_dist_pair(int n, t_point points[])
{
	...
	for (i=1; i<n-1; i++) {				// Outer loop picks one point
		for (j=i+1; j<n-1; j++) {		// Inner loop compares that chosen point with the remaining points, j = i+1 to avoid re-comparing (A B & B A)
			double distance = squared_dist( &(points[i]), &(points[j]) );
			if (distance < min_d) {
				min_d = distance;
				p_pair -> p1 = points[i];
				p_pair -> p2 = points[j];
