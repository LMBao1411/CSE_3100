...
int main(int argc, char *argv[])
{
    ...
    int b[10];    
    int cur = 0;        // current index that the walker is on and a[cur] is how far do i need to move, cur += a[cur] is moving right, cur -= a[cur] is moving left.
    int moves = 0;
    ...
    for(int i = 0; i< n; i++)
    {
        ...
        {   
            ...
            if(a[cur]==0)
            {
                b[moves - 1] = cur;     // when arriving at the value 0 in array a, save the cur index in the current nth steps of array b
                print_solution(b, moves);
                return 0;
            }
            else if(cur + a[cur] >= 0 && cur + a[cur] < n)      // the child is moving right
            {
                //Add your code here
                cur += a[cur];
                b[moves] = cur;
        ...
        else
        {
            ...
            else if(cur - a[cur] >= 0 && cur - a[cur] < n)      // the parent is moving left
            {
                //Add your code here
                cur -= a[cur];
                b[moves] = cur;
