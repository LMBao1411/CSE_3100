	...
	for(n=1; n<=1000000; n++) {
		...
		for(i=1;i<=100;i++) {
			for(j=i;j<=100;j++) {
				if (i*i*i + j*j*j == n) {
					count++;
					break;			// found the first pair (i1,j1) so we break for this i1 cycle
									// and we move on to the next pair of (i2,j2)
				}
			}
			if (count>=2) {
				found = 1;			// found the second pair (i2,j2)
			}
		}
		if (found) {				// since two pairs are found, we can terminate the loop
			break;
		}
	}
