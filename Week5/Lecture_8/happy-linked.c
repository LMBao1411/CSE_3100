// In this example, we use linked list to determine whether a positive integer
// is a happy number Given a positive integer, we repeatedly apply the following
// operation Summing up all the squared digits in the number, and the result is
// the next number For example, if the number is 19, then the next number is 1*1
// + 9*9 = 82 Then we apply the same operation on 82, we get 8*8 + 2*2 = 68,
// Then we will get 6*6 + 8*8 = 100
// Then 1*1 + 0*0 + 0*0 = 1
// For a positive integer, if we repeat the above operation and eventually reach
// 1, then the given number is a happy number. On the other hand, if at certain
// point, we reach a number that already seen before, then the given number is
// not a happy number.
