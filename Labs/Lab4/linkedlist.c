...
node *delete_node(node *head, int v) {
  ...
  // Case 2: If target is not found
  if (target == NULL) {
    error_message(ERR_NOTFOUND);
    return head;
  }

  // Case 3: If list only has the head node
  if (target == head) {
    node *temp = head;
    head = head->next;
    free(temp);
  ...
  // Case 4: Deleting a node in between nodes or the last one
...
node *reverse_list(node *head) {
  ...
  // Reverse the linked list iteratively by having prev, curr, and nxt pointers
  node *prev = NULL;
  node *curr = head;
  while (curr != NULL) {
    node *nxt = curr->next;
