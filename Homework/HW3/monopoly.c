...
{
	if (p1->balance >= amount) {		// successful transaction
		p1->balance -= amount;
		p2->balance += amount;
		return 1;
	}
...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
        ...
        p[i].loc += steps;
        if (p[i].loc >= n) {
            p[i].loc -= n;									// exceed the board range, substract n to wrap around
            p[i].balance += n;
        }

        if (prop[p[i].loc].owner_id == -1) {				//property is empty so the current player claim it
            prop[p[i].loc].owner_id = p[i].id;
        }
        else if (prop[p[i].loc].owner_id != p[i].id) {				// the property's owner id at the current player location VS that current player id
            if (transaction(&p[i], &p[prop[p[i].loc].owner_id], prop[p[i].loc].rent) == 0) {	// if the transaction fails, meaning game is over
                return 0;
            }
        }
	}
